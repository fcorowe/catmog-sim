...
A first quick and dirty approach we can take is to use a simple linear
regression model (LRM). This approach does not preserve the
multiplicative nature of @eq-1. Rather, it assumes linearity in the
relationship between the flows and the parameters of mass and distance.
...
production_constrained_poissonrm <- glm(count ~ ltla_res_code +  population_o + population_d + distance_km,
                 data = df,
                 family = poisson)

# Attraction contrained 
attraction_constrained_poissonrm <- glm(count ~ ltla_work_code +  population_o + population_d + distance_km,
                 data = df,
                 family = poisson)

# Doubly contrained 
doubly_constrained_poissonrm <- glm(count ~ ltla_res_code + ltla_work_code +  population_o + population_d + distance_km,
                 data = df,
                 family = poisson)
```
